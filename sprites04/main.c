...
void flush(void) {
	...
	glBufferData(GL_ARRAY_BUFFER, VERTEX_DATA_SIZE, pdata, GL_DYNAMIC_DRAW);
	glDrawArrays(GL_TRIANGLES, 0, count * VERTICES_PER_QUAD);
	count = 0;
}

/* TODO: add texture argument */
void draw(float x, float y, float w, float h, float r, float tx, float ty,
          float sx, float sy, float u0, float v0, float u1, float v1) {
